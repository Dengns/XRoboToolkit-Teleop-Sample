...
6. 再按一次空格键，机械臂停止跟随。
7. 跟随过程中按键盘 `↑` / `↓`，可以即时增大或减小 `xyz scale`，每次只调整 1 档，不需要回车。
8. 当前 `xyz scale` 预设档位是 `0.1` 到 `1.0`，步长 `0.1`。
9. 跟随过程中按数字键可以直接切档，不需要回车：
   - `1=0.1`
   - `2=0.2`
   - `3=0.3`
   - `4=0.4`
...
- 按 `↑` / `↓` 或数字键后出现 `tracker xyz scale 调整为 ...`：说明比例调整已经即时生效。
