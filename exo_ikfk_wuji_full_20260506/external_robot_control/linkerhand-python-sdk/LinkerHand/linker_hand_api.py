...
class LinkerHandApi:
    def __init__(self, hand_type="left", hand_joint="L10", modbus = "None",can="can0"):  # Ubuntu:can0   win:PCAN_USBBUS1
        ...
        if self.hand_joint.upper() == "O6":
            if modbus != "None":
                # 这里必须显式走 LinkerHand 包内路径，避免被仓库根目录同名 core/ 阴影。
                from LinkerHand.core.rs485.linker_hand_o6_rs485 import LinkerHandO6RS485
                self.hand = LinkerHandO6RS485(hand_id=self.hand_id,modbus_port=modbus,baudrate=115200)
            else:
                from LinkerHand.core.can.linker_hand_o6_can import LinkerHandO6Can
                self.hand = LinkerHandO6Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L6":
            if modbus != "None":
                from LinkerHand.core.rs485.linker_hand_l6_rs485 import LinkerHandL6RS485
                self.hand = LinkerHandL6RS485(hand_id=self.hand_id,modbus_port=modbus,baudrate=115200)
            else:
                from LinkerHand.core.can.linker_hand_l6_can import LinkerHandL6Can
        ...
        if self.hand_joint == "L10":
            if modbus != "None":
                from LinkerHand.core.rs485.linker_hand_l10_rs485 import LinkerHandL10RS485
        ...
        if self.hand_joint == "L21":
            from LinkerHand.core.can.linker_hand_l21_can import LinkerHandL21Can
            self.hand = LinkerHandL21Can(can_id=self.hand_id,can_channel=self.can, yaml=self.yaml)
        if self.hand_joint == "L25":
            from LinkerHand.core.can.linker_hand_l25_can import LinkerHandL25Can
    ...
    def finger_move(self, pose=[]):
        ...
        elif (self.hand_joint == "L7" or self.hand_joint == "O7") and len(pose) == 7:
            self.hand.set_joint_positions(pose)
    ...
    def set_speed(self, speed=[100]*5):
        ...
        if len(speed) < 5:
            ...
            return
        if (self.hand_joint == "L7" or self.hand_joint == "O7") and len(speed) < 7:
    ...
    def set_torque(self, torque=[180] * 5):
        ...
        if len(torque) < 5:
            print("数据长度不够,至少5个元素", flush=True)
            return
        if (self.hand_joint == "L7" or self.hand_joint == "O7") and len(torque) < 7:
            print("数据长度不够,至少7个元素", flush=True)
            return
    ...
    def get_joint_speed(self):
        ...
        if self.hand_joint.upper() == "O6" or self.hand_joint.upper() == "L6":
            return self.hand.get_speed()
        elif self.hand_joint == "L7" or self.hand_joint == "O7":
            return self.hand.get_speed()
